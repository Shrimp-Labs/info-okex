  ...
  const textContent = below800 ? (
    <div>
      <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
        Anyone can create and name any KIP20 token on OEC Chain, including creating fake versions of existing tokens and
        tokens that claim to represent projects that do not have a token.
      </Text>
      <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
        Similar to Etherscan, this site automatically tracks analytics for all KIP20 tokens independent of token
    ...
    <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
      ...
      tracks analytics for all KIP20 tokens independent of token integrity. Please do your own research before
      interacting with any KIP20 token.
    </Text>
  )
          ...
          <div>
            <Hover style={{ marginTop: '10px' }}>
              <Link
                ...
                target="_blank"
              >
                View {type === 'token' ? 'token' : 'pair'} contract on oec
              </Link>
            </Hover>
